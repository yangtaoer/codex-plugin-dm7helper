...
class PluginLayoutTest(unittest.TestCase):
    def test_required_plugin_files_and_names_match(self):
        ...
        self.assertIn(manifest["version"], bootstrap)
        self.assertIn("Security.Cryptography.SHA256", bootstrap)
        self.assertIn("expectedLauncher", bootstrap)
        self.assertIn("expectedJar", bootstrap)
        self.assertIn("dm7-mcp-snapshot-", bootstrap)
        self.assertIn("snapshot jar mismatch", bootstrap)
        self.assertNotIn("Sort-Object Name", bootstrap)
