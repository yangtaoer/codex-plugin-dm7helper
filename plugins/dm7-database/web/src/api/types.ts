...
export type HistoryQuery = Record<string, string | number | boolean | undefined>
export type HistoryItem = {
  executionId: string; correlationId: string; connectionFingerprint: string
  source: string; purpose: string | null; status: string; startedAt: string
...
export type EventRecord = { id: string; executionId: string; status: string; timestamp: string; detail: string }
...
export type ExecuteInput = { connectionId?: string; executionId?: string; sql: string; parameters?: SqlParameter[]; purpose: 'SCHEMA_CHANGE' | 'DATA_FIX' | 'MOCK' | 'TEST' | 'SEED' | 'SAMPLE'; atomic?: boolean; continueOnError?: boolean; timeoutSeconds?: number }
export type StatementResult = { index: number; kind: string; success: boolean; committed: boolean; rowCount: number; recorded: boolean; exclusionReason: string | null; commitBehavior: string; elapsedMillis: number; error: SafeExecutionError | null }
export type ExecuteResult = { executionId: string; success: boolean; status: string; statements: StatementResult[]; elapsedMillis: number; databaseFingerprint: string; error: SafeExecutionError | null }
export type MetadataQuery = Record<string, string | number | undefined>
...
export interface ApiClient {
  ...
  removeConnection(id: string, signal?: AbortSignal): Promise<void>
