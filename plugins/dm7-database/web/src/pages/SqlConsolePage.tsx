...
import type { ApiClient, EventRecord, ExecuteResult, QueryResult, SafeConnection, SafeExecutionError, SqlClassification, SqlPurpose } from '../api/types'
...
export function SqlConsolePage({api,events,streamStatus,theme,initialSql=''}:{api:ApiClient;events:EventRecord[];streamStatus:StreamStatus;theme:string;initialSql?:string}) {
  ...
  useEffect(()=>()=>{generation.current+=1;classifyRef.current?.abort()},[])
  ...
  useEffect(()=>{if(eventTerminal){setCancelPending(false);setHttpTerminal(true);setBusy(false)}},[eventTerminal])
  const cancellable=Boolean(executionId&&latest&&activeStates.has(latest.status)&&!cancelPending&&!httpTerminal)
  ...
  const startExecution=(id:string,current:ExecutionSnapshot)=>{activeExecution.current=id;setExecutionId(id);setSnapshot({...current,executionId:id});setTab('timeline')}
  const finishHttp=(token:number,id:string)=>{if(!isCurrent(token,id))return false;setCancelPending(false);setHttpTerminal(true);setBusy(false);return true}
  ...
  const run=async(candidate:string)=>{
    ...
    classifyRef.current?.abort();const controller=new AbortController();classifyRef.current=controller
    const token=++generation.current,current=captureSnapshot(token);activeExecution.current=undefined
    setSnapshot(current);setBusy(true);setError(null);setQueryResult(undefined);setExecuteResult(undefined);setExecutionId(undefined);setCancelPending(false);setHttpTerminal(false)
    try{
      ...
      if(classified.queryOnly){const id=uuid();if(!id)throw new Error('浏览器无法生成安全执行标识，已阻止执行。');startExecution(id,current)
        ...
        if(!finishHttp(token,id))return;if(result.executionId!==id)throw new Error('执行响应标识不匹配，结果已丢弃。');setQueryResult(result);setTab('result')
    ...
    }catch(e:unknown){if(isCurrent(token)&&!(e instanceof DOMException&&e.name==='AbortError')){setError({message:(e as Error).message||'SQL 分类或执行失败。',correlationId:(e as {correlationId?:string}).correlationId});setCancelPending(false);setHttpTerminal(Boolean(activeExecution.current));setBusy(false)}}
  }
  const confirmMutation=async()=>{
    ...
    try{const result=await api.execute({connectionId:current.connectionId,executionId:id,sql:pendingSql,purpose,atomic:classification.atomicAllowed&&atomic,continueOnError:!atomic&&continueOnError,timeoutSeconds:current.limits.timeoutSeconds})
      if(!finishHttp(current.token,id))return;if(result.executionId!==id)throw new Error('执行响应标识不匹配，结果已丢弃。');setExecuteResult(result);setTab('messages')
    }catch(e:unknown){if(isCurrent(current.token,id)){setError({message:(e as Error).message||'执行失败。',correlationId:(e as {correlationId?:string}).correlationId});setCancelPending(false);setHttpTerminal(true);setBusy(false)}}finally{if(isCurrent(current.token,id))setPendingSql('')}
  }
  const cancel=async()=>{const id=executionId,token=generation.current;if(!id||!cancellable)return;setCancelPending(true);try{const result=await api.cancelExecution(id);if(!isCurrent(token,id))return;if(!result.cancelRequested){setError({message:'任务已结束，取消请求未生效。'});setCancelPending(false)}}catch(e:unknown){if(isCurrent(token,id)){setError({message:(e as Error).message||'无法请求取消。',correlationId:(e as {correlationId?:string}).correlationId});setCancelPending(false)}}}
  ...
  return <><PageHeader eyebrow="MANUAL SQL" title="SQL 执行控制台" description="先由 DM 语法解析器分类，再通过已知执行标识跟踪全过程。" />
    ...
    <section className="sql-output"><div className="output-tabs" role="tablist" aria-label="SQL 输出">{tabs.map(item=><button key={item.id} ref={element=>{tabRefs.current[item.id]=element}} id={`sql-tab-${item.id}`} role="tab" tabIndex={tab===item.id?0:-1} aria-selected={tab===item.id} aria-controls={`sql-panel-${item.id}`} onClick={()=>selectTab(item.id)} onKeyDown={event=>tabKey(event,item.id)}>{item.label}</button>)}</div>
      ...
      <div id="sql-panel-timeline" role="tabpanel" aria-labelledby="sql-tab-timeline" hidden={tab!=='timeline'}><ExecutionTimeline executionId={executionId} events={events} streamStatus={streamStatus}/></div>
    </section>
...
export function MutationMessages({result}:{result:ExecuteResult}){return <div className="mutation-messages"><div className="result-summary"><span>{result.success?'执行完成':'部分或全部失败'}</span><span>{result.elapsedMillis} ms</span><span>DB {result.databaseFingerprint.slice(0,12)}</span></div>{result.error&&<SafeError error={result.error}/>}<ol>{result.statements.map(item=>{const repeatsOverall=Boolean(result.error&&item.error?.correlationId===result.error.correlationId);return <li key={item.index}><strong>#{item.index+1} · {item.kind}</strong><span>{item.success?'成功':'失败'} · {item.committed?'已提交':'未提交'}（{item.commitBehavior}）· {item.rowCount} 行 · {item.elapsedMillis} ms · {item.recorded?'已记录':item.exclusionReason??'未记录'}</span>{item.error&&!repeatsOverall&&<SafeError error={item.error}/>}</li>})}</ol></div>}
