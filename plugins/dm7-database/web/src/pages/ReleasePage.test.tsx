...
it('shows complete artifact facts and copies SHA with accessible feedback',async()=>{const writeText=vi.fn().mockResolvedValue(undefined);Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText}});render(<ReleasePage api={{release:vi.fn().mockResolvedValue(snapshot('v002',0))} as unknown as ApiClient}/>);await screen.findByText('dm7-abc-v001.sql');expect(screen.getByText(/128 B · 序列 1–2/)).toBeTruthy();fireEvent.click(screen.getByRole('button',{name:'复制 v001 SHA-256'}));await waitFor(()=>expect(writeText).toHaveBeenCalledWith('b'.repeat(64)));expect(screen.getByText('SHA-256 已复制')).toBeTruthy()})

it('surfaces SHA copy failure without exposing clipboard data',async()=>{Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText:vi.fn().mockRejectedValue(new Error('denied'))}});render(<ReleasePage api={{release:vi.fn().mockResolvedValue(snapshot('v002',0))} as unknown as ApiClient}/>);await screen.findByText('dm7-abc-v001.sql');fireEvent.click(screen.getByRole('button',{name:'复制 v001 SHA-256'}));expect(await screen.findByText('SHA-256 复制失败')).toBeTruthy();expect(document.body.textContent).not.toContain('denied')})

it('never offers recovery for missing tampered or unavailable sources',async()=>{for(const integrityState of ['MISSING','TAMPERED','UNAVAILABLE']){const artifact={...snapshot('v002',0).artifacts[0],state:'RECOVERY_REQUIRED' as const,integrityState,downloadAvailable:false};const {unmount}=render(<ReleasePage api={{release:vi.fn().mockResolvedValue({...snapshot('v002',0),artifacts:[artifact]})} as unknown as ApiClient}/>);await screen.findByText(integrityState);expect(screen.queryByRole('button',{name:'恢复导出'})).toBeNull();unmount()}})
