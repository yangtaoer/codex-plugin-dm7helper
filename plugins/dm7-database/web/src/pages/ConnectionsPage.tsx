...
export function ConnectionsPage({ api }: { api: ApiClient }) {
  ...
  const openDrawer = (mode: Mode, source?: SafeConnection) => {
    ...
    setForm(next); setInitialForm(next); setDrawer({ mode, source }); setFieldErrors({}); setDiagnostics(undefined)
    setAdvanced(false); setShowPassword(false)
  }
  const closeNow = () => {
    setForm(emptyForm()); setInitialForm(emptyForm()); setDrawer(undefined); setDiagnostics(undefined); setFieldErrors({})
  ...
  const validate = () => {
    ...
    setFieldErrors(issues); firstInvalid.current = Object.keys(issues)[0] ?? null
  ...
  const submit = async (testAfter = false) => {
    ...
    } catch (cause) { setError(safeError(cause, '连接保存失败。')); setForm((current) => ({ ...current, password: '' })) }
  ...
  const remove = async () => {
    ...
    try { await api.removeConnection(id); setDeleteTarget(undefined); await refresh(false) }
  ...
  return <>
    ...
    {drawer && <aside ref={drawerRef} className="connection-drawer" role="dialog" aria-modal="true" aria-hidden={unsavedOpen || undefined} aria-labelledby="connection-editor-title"><div className="drawer-heading"><div><span className="eyebrow">{drawer.mode.toUpperCase()} PROFILE</span><h2 id="connection-editor-title">{drawer.mode === 'create' ? '新增连接' : drawer.mode === 'copy' ? '复制连接' : '编辑连接'}</h2></div><button className="icon-button" aria-label="关闭连接编辑器" onClick={requestClose}><X size={19} /></button></div><form onSubmit={(event) => { event.preventDefault(); void submit(false) }} noValidate>
      <div className="drawer-scroll"><Field id="name" label="连接名称" value={form.name} error={fieldErrors.name} onChange={(v) => update('name', v)} required />
      ...
      <div className="form-field"><label htmlFor="password">密码</label>{drawer.source && <small>{drawer.source.hasPassword ? '已配置；留空保留' : '未配置'}</small>}<div className="password-input"><input id="password" type={showPassword ? 'text' : 'password'} autoComplete="new-password" value={form.password} disabled={form.clearPassword} onChange={(event) => update('password', event.target.value)} /><button type="button" aria-label={showPassword ? '隐藏密码' : '显示密码'} aria-pressed={showPassword} onClick={() => setShowPassword((v) => !v)}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div>{drawer.mode === 'edit' && drawer.source?.hasPassword && <label className="check-line"><input type="checkbox" checked={form.clearPassword} onChange={(event) => { update('clearPassword', event.target.checked); if (event.target.checked) update('password', '') }} />清除已保存密码</label>}</div>
    ...
    <ConfirmDialog open={Boolean(deleteTarget)} title="删除数据库连接" confirmLabel="确认删除" onConfirm={() => void remove()} onClose={() => setDeleteTarget(undefined)}><p>将删除“{deleteTarget?.name}”及其已保存凭据。</p>{deleteTarget?.isDefault && <p className="danger-copy">这是默认连接，删除后系统将选择替代项或进入无默认状态。</p>}</ConfirmDialog>
