...
export function ResultGrid({ result }: { result: QueryResult }) {
  ...
  const visible=virtualized?virtual.getVirtualItems().map(item=>({row:rows[item.index],position:item.start,index:item.index})):rows.map((row,index)=>({row,position:0,index}))
  const leafColumns=table.getAllLeafColumns(),operationWidth=84
  const trackSizes=[...leafColumns.map(column=>`minmax(${column.getSize()}px,1fr)`),`${operationWidth}px`]
  const tableWidth=leafColumns.reduce((total,column)=>total+column.getSize(),operationWidth)
  const nonce=document.querySelector<HTMLMetaElement>('meta[name="csp-nonce"]')?.content
  const dynamicCss=`.result-scroll{height:${Math.min(440,Math.max(120,rows.length*38+42))}px}`+
    `.result-table{width:${tableWidth}px;min-width:100%}.result-table tr{grid-template-columns:${trackSizes.join(' ')}}`+
    (virtualized?`.result-table tbody{height:${virtual.getTotalSize()}px;position:relative}`+visible.map(({row,position})=>`.result-table tr[data-row="${row.id}"]{position:absolute;transform:translateY(${position}px);width:100%}`).join(''):'')
  return <section className="result-panel">{nonce&&<style nonce={nonce}>{dynamicCss}</style>}<div className="result-summary"><span>{result.returnedRows} 行</span><span>{result.elapsedMillis} ms</span><span>{result.bytes} bytes</span><span title={result.databaseFingerprint}>DB {result.databaseFingerprint.slice(0,12)}</span>{result.truncated&&<strong>结果已截断</strong>}<button disabled={!result.success||!result.rows.length} onClick={()=>save(safeName('csv'),csvText(result),'text/csv;charset=utf-8')}>下载 CSV</button><button disabled={!result.success||!result.rows.length} onClick={()=>save(safeName('json'),jsonText(result),'application/json;charset=utf-8')}>下载 JSON</button></div>
    ...
    <div ref={parent} className="result-scroll"><table className="result-table" data-virtualized={virtualized}><thead>{table.getHeaderGroups().map(group=><tr key={group.id}>{group.headers.map(header=><th key={header.id}><button onClick={header.column.getToggleSortingHandler()}>{flexRender(header.column.columnDef.header,header.getContext())}</button><span role="separator" aria-label={`调整 ${header.id} 列宽`} onMouseDown={header.getResizeHandler()} /></th>)}<th>操作</th></tr>)}</thead><tbody>{visible.map(({row,index})=><tr key={row.id} ref={virtualized?virtual.measureElement:undefined} data-index={virtualized?index:undefined} data-row={row.id}>{row.getVisibleCells().map(cell=><td key={cell.id} tabIndex={0} onDoubleClick={()=>void copy(printable(cell.getValue()))}>{flexRender(cell.column.columnDef.cell,cell.getContext())}<button className="cell-copy" aria-label={`复制第 ${row.index+1} 行${cell.column.id}单元格`} onClick={()=>void copy(printable(cell.getValue()))}>复制</button></td>)}<td><button className="row-copy" aria-label={`复制第 ${row.index+1} 行`} onClick={()=>void copy(result.columns.map(column=>printable(row.original[column.outputLabel])).join('\t'))}>复制行</button></td></tr>)}</tbody></table></div><div className="sr-only" aria-live="polite">{copyStatus}</div></section>
}
