...
export function ExecutionTimeline({ executionId, events, streamStatus, authoritativeElapsedMillis }: { executionId?: string; events: EventRecord[]; streamStatus: StreamStatus; authoritativeElapsedMillis?:number }) {
  ...
  return <section className="execution-timeline" aria-label="实时过程">
    ...
    {authoritativeElapsedMillis!==undefined&&<p className="timeline-authoritative">执行总耗时 {format(authoritativeElapsedMillis)} ms</p>}
    {!executionId?<p className="quiet-copy">执行后将在此显示十阶段状态。</p>:!selected.length?<p className="quiet-copy">等待执行事件；当前没有可见的耗时窗口。</p>:<><ol>{selected.map((event,index)=>{const at=millis(event.timestamp),previous=index?millis(selected[index-1].timestamp):at;return <li key={event.id||`${event.timestamp}-${event.status}`}><code>{labels[event.status]??event.status.toUpperCase()}</code><span className="event-sequence">SEQ {event.id}</span><time dateTime={event.timestamp}>{new Date(at).toLocaleString('zh-CN',{hour12:false})}</time><span>{event.detail}</span><small>+{format(at-previous)} ms · 已观察 {format(at-started)} ms</small></li>})}</ol>{terminalIndex>=0&&<p className="timeline-total">已观察耗时 {format(total)} ms · 可见事件窗口 · {labels[selected.at(-1)!.status]}</p>}</>}
  </section>
}
