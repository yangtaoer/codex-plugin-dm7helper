...
test('tabs rove with arrows and dialogs loop focus without losing the trigger', async ({ page }) => {
  await page.goto('/app/sql')
  const resultsTab = page.getByRole('tab', { name: '结果表格' })
  await resultsTab.focus(); await page.keyboard.press('ArrowRight')
  await expect(page.getByRole('tab', { name: '执行消息' })).toBeFocused()
  await page.keyboard.press('End')
  await expect(page.getByRole('tab', { name: '实时过程' })).toBeFocused()
  await page.goto('/app/connections')
  const trigger = page.getByRole('button', { name: '编辑华东生产只读' })
  await trigger.click()
  const dialog = page.getByRole('dialog')
  const close = dialog.getByRole('button', { name: '关闭连接编辑器' })
  await expect(close).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(dialog.getByRole('button', { name: '保存更改' })).toBeFocused()
  await page.keyboard.press('Tab')
  await expect(close).toBeFocused()
  await page.keyboard.press('Escape')
